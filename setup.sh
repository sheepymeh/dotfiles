yay -S --noconfirm acpi alsa-utils android-tools arc-gtk-theme base bash-completion code cups-pdf dialog dmidecode efibootmgr exfat-utils firefox grim grub gvfs i3blocks intel-ucode light lightdm linux linux-firmware lollypop lsof mako nano neofetch networkmanager nginx-mainline npm ntfs-3g p7zip pacman-contrib papirus-icon-theme php-fpm pulseaudio-alsa pulseaudio-bluetooth qbittorrent slurp sway termite throttled thunar-archive-plugin thunar-volman ttf-font-awesome ttf-roboto ttf-roboto-mono unzip wget wl-clipboard xbindkeys xdg-user-dirs xf86-video-intel xf86-video-nouveau xorg-server xorg-server-xwayland xorg-xrandr youtube-dl
...
yay -S --noconfirm sudo lightdm-mini-greeter noto-fonts-sc plymouth plymouth-theme-arch-agua postman-bin wob wofi-hg
# mongodb?
