yay -Syu
yay -S --noconfirm --needed acpi alsa-utils android-tools arc-gtk-theme avahi base bash-completion bbswitch code cups-pdf ttf-dejavu dialog dmidecode efibootmgr exfat-utils firefox gnome-keyring grim grub gst-plugins-good gvfs gvfs-mtp i3blocks imv intel-ucode light linux linux-firmware lollypop lsof mako nano neofetch networkmanager nextcloud-client npm ntfs-3g p7zip pacman-contrib papirus-icon-theme php-fpm pulseaudio-alsa pulseaudio-bluetooth qbittorrent qt5-wayland slurp sway swayidle swaylock termite throttled thunar thunar-archive-plugin ttf-font-awesome ttf-roboto ttf-roboto-mono unzip wget wl-clipboard xbindkeys xdg-user-dirs xf86-video-intel xf86-video-nouveau xorg-server xorg-server-xwayland xorg-xrandr youtube-dl
...
yay -S --noconfirm --needed mongodb-bin noto-fonts-sc plymouth plymouth-theme-arch-agua postman-bin wob wofi-hg
yay -D --asexplicit go
